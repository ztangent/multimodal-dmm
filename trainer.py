...
class Trainer(object):
    """Abstract class for training on multimodal sequential data."""    
    
    def train(self, loader, model, optimizer, epoch, args):
        model.train()
        loss = 0.0
        data_num = 0
    ...
    def evaluate(self, loader, model, args):
        model.eval()
        # Set up accumulators
        n_timesteps = 0
        metrics = None
    ...
    def save_params(self, args, model):
    ...
    def run(self, args):
        ...
        train_data, test_data = self.load_data(args.modalities, args)
        ...
        if hasattr(models, args.model):
            ...
            model = self.build_model(constructor, args)
        ...
        args = self.default_args(args, model)
        ...
        if args.test:            
            ...
            with torch.no_grad():
                args.eval_set = 'train'
                results, _  = self.evaluate(eval_loader, model, args)
        ...
        if args.find_best:
            ...
            for epoch in range(args.save_freq, args.epochs+1, args.save_freq):
                ...
                with torch.no_grad():
                    _, metrics = self.evaluate(test_loader, model, args)
            ...
            model.load_state_dict(checkpoint['model'])
            with torch.no_grad():
                self.evaluate(test_loader, model, args)
        ...
        for epoch in range(1, args.epochs + 1):
            print('---')
            self.train(train_loader, model, optimizer, epoch, args)
